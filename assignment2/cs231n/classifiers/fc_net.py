...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        last_size=0
        for i, size in enumerate(hidden_dims,start=1):
            if i==1:
                self.params[f'w{i}']=weight_scale*np.random.randn(input_dim,size)
            else:
                self.params[f'w{i}']=weight_scale*np.random.randn(last_size,size)
            last_size=size
