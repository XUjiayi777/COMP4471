...
def svm_loss_vectorized(W, X, y, reg):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    matrix = np.zeros((X.shape[0], W.shape[1]))
    matrix[range(X.shape[0]), y] = 0 #correct class
    matrix [margin > 0] = 1 #incorrect class
    matrix[range(X.shape[0]), y] = -np.sum(matrix, axis=1)
    dW = (X.T).dot(matrix)
